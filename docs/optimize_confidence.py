...
def load_eval_results(path: str) -> tuple[list[dict], int]:
    ...
    Returns (usable_records, n_not_found).
    not_found records are excluded: they have no composite score to optimize
    (not_found detection is a binary flag, not a quality tier signal).
    """
    with open(path) as f:
        records = json.load(f)

    usable = [r for r in records if not r.get("not_found", False)]
    n_not_found = len(records) - len(usable)
    return usable, n_not_found
